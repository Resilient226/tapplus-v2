...
// ── Home ──────────────────────────────────────────────────────────────────────
function renderHome(){
app().innerHTML=` <div class="page-center"> <div style="font-size:56px;font-weight:900;letter-spacing:-.04em;margin-bottom:4px">Tap<span style="color:var(--green)">+</span></div> <div style="font-size:14px;color:var(--gray);margin-bottom:40px">Enter your store code</div> <div style="width:100%;max-width:320px"> <input class="inp" id="code-inp" placeholder="4-digit code" type="number" inputmode="numeric" maxlength="4" style="text-align:center;font-size:32px;font-weight:900;letter-spacing:.2em;padding:18px;margin-bottom:12px" onkeydown="if(event.key==='Enter')window._go()"/> <button class="btn btn-primary btn-full" onclick="window._go()">Continue →</button> <button class="btn btn-ghost btn-full" style="margin-top:10px" onclick="window._ownerEntry()"> Owner / Create Account </button> <div style="margin-top:24px;text-align:center"> <button onclick="window._sa()" style="background:none;border:none;color:rgba(238,240,248,.12);font-size:11px;cursor:pointer;font-family:'Nunito',sans-serif">●</button> </div> </div> </div>`;
...
window._sa=function(){
showModal(`<div class="modal-head"><div class="modal-title">Super Admin</div><button class="modal-close" onclick="closeModal()">×</button></div> <div style="text-align:center;padding:8px 0"> <div style="font-size:18px;font-weight:800;margin-bottom:16px">Enter PIN</div> <div class="pin-display" style="gap:10px"> <div class="pin-dot" id="pd0"></div><div class="pin-dot" id="pd1"></div> <div class="pin-dot" id="pd2"></div><div class="pin-dot" id="pd3"></div> <div class="pin-dot" id="pd4"></div><div class="pin-dot" id="pd5"></div> </div> <div class="pin-grid">${['1','2','3','4','5','6','7','8','9','del','0','go'].map(k=>`<button class="pin-key${k==='del'||k==='go'?' del':''}" onclick="window._pin('${k}')" style="${k==='go'?'background:var(--green);color:var(--black);border-color:var(--green)':''}">${k===‘del’?‘⌫’:k===‘go’?‘→’:k}</button>`).join('')}</div> </div>`);
let pin=’’;
...
const biz=State.biz;
app().innerHTML=` <div class="page-center"> ${biz.branding?.logoUrl?`<img src="${esc(biz.branding.logoUrl)}" style="height:60px;object-fit:contain;border-radius:12px;margin-bottom:20px"/>`:`<div style="font-size:24px;font-weight:900;margin-bottom:20px">${esc(biz.name)}</div>`} <div style="font-size:14px;color:var(--gray);margin-bottom:28px">Who are you?</div> <div style="width:100%;max-width:320px;display:flex;flex-direction:column;gap:10px"> ${[['staff','👤','Staff Member','View my stats & feedback'],['manager','📊','Manager','Team overview & analytics'],['bizAdmin','⚙️','Business Admin','Full settings & control'],['owner','🔑','Owner','Sign in with email']].map(([r,ic,lbl,sub])=>`
...
</button>`).join('')} </div> <button onclick="renderHome()" style="margin-top:20px;background:none;border:none;color:var(--gray);font-size:13px;cursor:pointer;font-family:'Nunito',sans-serif">← Back</button> </div>`;
...
app().innerHTML=` <div class="page-center"> <div style="font-size:13px;color:var(--gray);margin-bottom:8px">${esc(State.biz?.name)}</div> <div style="font-size:20px;font-weight:800;margin-bottom:4px">${titles[role]}</div> <div style="font-size:13px;color:var(--gray);margin-bottom:20px">${subs[role]}</div> <div class="pin-display" style="gap:10px"> <div class="pin-dot" id="pd0"></div><div class="pin-dot" id="pd1"></div> <div class="pin-dot" id="pd2"></div><div class="pin-dot" id="pd3"></div> <div class="pin-dot" id="pd4"></div><div class="pin-dot" id="pd5"></div> </div> <div class="pin-grid">${['1','2','3','4','5','6','7','8','9','del','0','go'].map(k=>`<button class="pin-key${k==='del'||k==='go'?' del':''}" onclick="window._pin('${k}')" style="${k==='go'?'background:var(--green);color:var(--black);border-color:var(--green)':''}">${k===‘del’?‘⌫’:k===‘go’?‘→’:k}</button>`).join('')}</div> <button onclick="renderRoleSelect()" style="margin-top:24px;background:none;border:none;color:var(--gray);font-size:13px;cursor:pointer;font-family:'Nunito',sans-serif">← Back</button> </div>`;
...
app().innerHTML=` <div class="page-center"> <div style="font-size:24px;font-weight:900;margin-bottom:8px">Owner Sign In</div> <div style="font-size:14px;color:var(--gray);margin-bottom:28px">Sign in with your email</div> <div style="width:100%;max-width:320px;display:flex;flex-direction:column;gap:12px"> <div><div class="field-lbl">Email</div><input class="inp" id="oe" type="email" placeholder="you@business.com"/></div> <div><div class="field-lbl">Password</div><input class="inp" id="op" type="password" placeholder="••••••••" onkeydown="if(event.key==='Enter')window._signin()"/></div> <button class="btn btn-primary btn-full" onclick="window._signin()">Sign In</button> <button class="btn btn-ghost btn-full" onclick="window._register()">Create Account</button> <button onclick="renderRoleSelect()" style="background:none;border:none;color:var(--gray);font-size:13px;cursor:pointer;font-family:'Nunito',sans-serif;margin-top:4px">← Back</button> </div> </div>`;
...
app().innerHTML=` <div class="page" style="padding-top:60px"> <button onclick="renderOwnerLogin()" style="background:none;border:none;color:var(--gray);font-size:13px;cursor:pointer;font-family:'Nunito',sans-serif;margin-bottom:20px;padding:0">← Back</button> <h1 style="margin-bottom:6px">Create Business</h1> <div style="color:var(--gray);font-size:14px;margin-bottom:24px">Set up your Tap+ location</div> <div style="display:flex;flex-direction:column;gap:12px"> <div><div class="field-lbl">Business Name</div><input class="inp" id="cb-n" placeholder="The James Room"/></div> <div><div class="field-lbl">Admin PIN (4-6 digits)</div><input class="inp" id="cb-a" type="text" inputmode="numeric" pattern="[0-9]*" placeholder="e.g. 1234"/></div> <div><div class="field-lbl">Manager PIN (4-6 digits)</div><input class="inp" id="cb-m" type="text" inputmode="numeric" pattern="[0-9]*" placeholder="e.g. 5678"/></div> <button class="btn btn-primary btn-full" style="margin-top:8px" onclick="window._create()">Create →</button> </div> </div>`;
...
app().innerHTML=` <div style="max-width:480px;margin:0 auto;padding:16px 16px 90px"> <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;padding-top:8px"> <div> ${biz.branding?.logoUrl?`<img src="${esc(biz.branding.logoUrl)}" style="height:30px;object-fit:contain;border-radius:6px"/>`:`<div style="font-size:17px;font-weight:900">${esc(biz.name)}</div>`} <div style="font-size:10px;color:var(--gray);font-weight:700;margin-top:2px">${role==='staff'&&me?esc(staffDisplay(me)):role.toUpperCase()}</div> </div> <button onclick="window._logout()" style="background:rgba(255,255,255,.06);border:1px solid var(--border);border-radius:8px;padding:6px 12px;color:var(--gray);font-size:12px;font-weight:700;cursor:pointer;font-family:'Nunito',sans-serif">Sign Out</button> </div> <div class="tabs">${sections.map(s=>`<button class="tab${s===active?' active':''}" onclick="window._tab('${s}')" id="tab-${s}">${LABELS[s]||s}</button>`).join('')}</div> <div id="dash-body"></div> </div> <div class="nav-bar"> <div class="nav-item active"><div class="nav-icon">📊</div><div>Dashboard</div></div> <div class="nav-item" onclick="window._preview()"><div class="nav-icon">👁</div><div>Preview</div></div> <div class="nav-item" onclick="window._logout()"><div class="nav-icon">🚪</div><div>Out</div></div> </div>`;
...
app().innerHTML=` <div style="position:fixed;top:0;left:0;right:0;z-index:100; background:rgba(7,8,12,.95);backdrop-filter:blur(10px); border-bottom:1px solid var(--border);padding:12px 16px; display:flex;align-items:center;gap:12px"> <button onclick="renderDashboard()" style="background:rgba(255,255,255,.08); border:1px solid var(--border);border-radius:8px;padding:7px 14px; color:var(--white);font-size:13px;font-weight:700;cursor:pointer; font-family:'Nunito',sans-serif">← Back</button> <div style="font-size:13px;font-weight:700;color:var(--gray)">Preview Mode</div> </div> <div style="padding-top:56px"> <div class="tap-page"> <div style="margin-top:16px;margin-bottom:24px;text-align:center"> ${b.logoUrl?`<img src="${esc(b.logoUrl)}" style="height:80px;max-width:220px;object-fit:contain;border-radius:16px"/>`:`<div style="font-size:28px;font-weight:900">${esc(b.name||‘Your Business’)}</div>`} ${b.tagline?`<div style="font-size:13px;opacity:.4;margin-top:8px">${esc(b.tagline)}</div>`:''} </div> <div style="text-align:center;margin-bottom:28px;width:100%"> <div style="font-size:20px;font-weight:900;margin-bottom:20px">${esc(b.ratingQuestion||'How was your experience today?')}</div> <div style="display:flex;gap:10px;justify-content:center"> ${[1,2,3,4,5].map(i=>`<div id="pcs${i}" style="font-size:42px;cursor:pointer;transition:transform .15s;filter:grayscale(1);opacity:.3" onclick="window._pStar(${i})">★</div>`).join('')} </div> </div> <div id="p-after" style="width:100%"></div> ${bulletinLinks.length?`<div style="width:100%;margin-top:16px"><div style="font-size:10px;font-weight:700;opacity:.3;letter-spacing:.1em;text-transform:uppercase;margin-bottom:12px;text-align:center">${esc(b.name)}</div>
${bulletinLinks.map(l=>`<div style="background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.08);border-radius:16px;padding:14px 18px;margin-bottom:10px"> <div style="font-weight:700;font-size:14px">${esc(l.label)}</div> ${l.sublabel?`<div style="font-size:12px;opacity:.5;margin-top:4px">${esc(l.sublabel)}</div>`:''}</div>`).join(’’)}
</div>`:''} ${links.length?`<div style="width:100%;margin-top:8px;background:rgba(255,255,255,.03);border:1px solid var(--border);border-radius:12px;padding:12px;text-align:center;color:var(--gray);font-size:13px">
${links.length} review link${links.length>1?‘s’:’’} configured — shown after 4-5★ rating
</div>`:''} <div style="position:fixed;bottom:10px;left:0;right:0;text-align:center;font-size:9px;font-weight:700;letter-spacing:.18em;text-transform:uppercase;opacity:.08;pointer-events:none">POWERED BY TAP+</div> </div> </div>`;
...
body.innerHTML=`<div class="plain-card"> <div style="font-weight:700;font-size:15px;margin-bottom:16px">✨ My Tap Page</div> <div class="field-lbl">Profile Photo</div> <div style="display:flex;align-items:center;gap:12px;margin-bottom:16px"> <div id="br-av">${staffAvatar(me,64)}</div> <button onclick="window._pickPhoto()" class="btn btn-ghost btn-sm">📷 Upload</button> </div> <div class="field-lbl">My Title</div> <input class="inp" id="br-title" value="${esc(me.title||'')}" placeholder="Server, Bartender" style="margin-bottom:14px"/> ${types.length?`
...
showModal(`<div class="modal-head"><div class="modal-title">Edit ${esc(staffDisplay(s))}</div><button class="modal-close" onclick="closeModal()">×</button></div> <div style="display:flex;flex-direction:column;gap:12px"> <div><div class="field-lbl">First Name</div><input class="inp" id="es-fn" value="${esc(s.firstName)}"/></div> <div><div class="field-lbl">Last Initial</div><input class="inp" id="es-li" value="${esc(s.lastInitial)}" maxlength="1"/></div> <div><div class="field-lbl">Title</div><input class="inp" id="es-ti" value="${esc(s.title||'')}"/></div> <div><div class="field-lbl">New Passcode (blank to keep)</div><input class="inp" id="es-pa" type="number" inputmode="numeric" placeholder="Leave blank" maxlength="4"/></div> <button class="btn btn-primary btn-full" onclick="window._updateS('${id}')">Save</button> <button class="btn btn-danger btn-full" onclick="window._delS('${id}')">Delete</button> </div>`);
...
let links=[…(biz?.links||[])];
function draw(){
body.innerHTML=`<div class="sec-lbl">Review Links</div> <div style="font-size:12px;color:var(--gray);margin-bottom:12px">4-5★ ratings redirect here</div> ${links.map((l,i)=>`<div class="plain-card" style="display:flex;align-items:center;gap:12px"><div style="flex:1;min-width:0"><div style="font-weight:700;font-size:14px">${esc(l.label||l.platform)}</div><div style="font-size:11px;color:var(--gray);overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${esc(l.url)}</div></div><button onclick="window._rmL(${i})" style="background:rgba(255,68,85,.08);border:1px solid rgba(255,68,85,.2);border-radius:7px;padding:5px 9px;font-size:11px;font-weight:700;color:var(--red);cursor:pointer;font-family:'Nunito',sans-serif">✕</button></div>`).join('')} <button class="btn btn-ghost btn-full" onclick="window._addL()" style="margin-top:4px">+ Add Link</button> <button class="btn btn-primary btn-full" onclick="window._saveL()" style="margin-top:8px">Save Links</button>`;
}
window._rmL=function(i){links.splice(i,1);draw();};
window._addL=function(){
showModal(`<div class="modal-head"><div class="modal-title">Add Review Link</div><button class="modal-close" onclick="closeModal()">×</button></div> <div style="display:flex;flex-direction:column;gap:12px"> <div><div class="field-lbl">Platform</div><select class="sel" id="al-p"><option>Google</option><option>Yelp</option><option>TripAdvisor</option><option>OpenTable</option><option>Custom</option></select></div> <div><div class="field-lbl">Label</div><input class="inp" id="al-l" placeholder="Review us on Google"/></div> <div><div class="field-lbl">URL</div><input class="inp" id="al-u" placeholder="https://g.page/…"/></div> <button class="btn btn-primary btn-full" onclick="window._doAddL()">Add</button> </div>`);
...
body.innerHTML=`<div class="plain-card"> <div style="font-weight:700;font-size:15px;margin-bottom:16px">🎨 Branding & Settings</div> <div class="field-lbl">Business Name</div><input class="inp" id="s-name" value="${esc(b.name||State.biz?.name)}" style="margin-bottom:10px"/> <div class="field-lbl">Tagline</div><input class="inp" id="s-tag" value="${esc(b.tagline||'')}" placeholder="We Create Memories" style="margin-bottom:10px"/> <div class="field-lbl">Logo URL or Upload</div> <div style="display:flex;gap:8px;margin-bottom:10px"><input class="inp" id="s-logo" value="${esc(b.logoUrl||'')}" placeholder="https://…" style="flex:1"/><button onclick="window._pickLogo()" class="btn btn-ghost btn-sm">📷</button></div> ${b.logoUrl?`<img src="${esc(b.logoUrl)}" id="s-logo-prev" style="height:48px;object-fit:contain;border-radius:8px;margin-bottom:10px;display:block"/>`:’’}
...
showModal(`<div class="modal-head"><div class="modal-title">Add Bulletin Item</div><button class="modal-close" onclick="closeModal()">×</button></div> <div style="display:flex;flex-direction:column;gap:12px"> <div><div class="field-lbl">Type</div><select class="sel" id="bl-t" onchange="window._blTog(this.value)"><option value="custom">🔗 Link</option><option value="text">📝 Text Only</option><option value="spotify">🎵 Spotify</option></select></div> <div><div class="field-lbl">Title</div><input class="inp" id="bl-l" placeholder="e.g. Happy Hour 4-6pm"/></div> <div id="bl-uw"><div class="field-lbl">URL</div><input class="inp" id="bl-u" placeholder="https://…"/></div> <div><div class="field-lbl">Description (optional)</div><input class="inp" id="bl-s" placeholder="More details…"/></div> <button class="btn btn-primary btn-full" onclick="window._doAddBull()">Add</button> </div>`);
...
app().innerHTML=` <div style="max-width:480px;margin:0 auto;padding:20px 16px 80px"> <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:24px;padding-top:8px"> <div style="font-size:22px;font-weight:900">Tap<span style="color:var(--green)">+</span> Owner</div> <button onclick="API.auth.logout();renderHome()" style="background:rgba(255,255,255,.06);border:1px solid var(--border);border-radius:8px;padding:6px 12px;color:var(--gray);font-size:12px;font-weight:700;cursor:pointer;font-family:'Nunito',sans-serif">Sign Out</button> </div> ${bizs.length===0?`<div class="card" style="text-align:center;padding:40px">
...
<button class="btn btn-ghost btn-full" style="margin-top:8px" onclick="renderCreateBusiness('${esc(sess.token)}')">+ Add Location</button>`} </div>`;
...
app().innerHTML=` <div style="max-width:480px;margin:0 auto;padding:20px 16px 80px"> <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:24px;padding-top:8px"> <div style="font-size:20px;font-weight:900">⚡ Super Admin</div> <button onclick="API.auth.logout();renderHome()" style="background:rgba(255,255,255,.06);border:1px solid var(--border);border-radius:8px;padding:6px 12px;color:var(--gray);font-size:12px;font-weight:700;cursor:pointer;font-family:'Nunito',sans-serif">Sign Out</button> </div> <div class="tabs"> <button class="tab active" onclick="window._saT('layout')" id="sa-layout">Layout</button> <button class="tab" onclick="window._saT('biz')" id="sa-biz">Businesses</button> </div> <div id="sa-body"></div> </div>`;
...
body.innerHTML = `<button class="btn btn-primary btn-full" style="margin-bottom:16px" onclick="window._saCreateBiz()"> + Create New Business </button> <div style="display:flex;gap:8px;margin-bottom:16px"> <input class="inp" id="sa-biz-search" placeholder="Search by slug…" style="flex:1" oninput="window._saSearch(this.value)"/> </div> <div id="sa-biz-list"> ${businesses.length === 0 ? '<div class="card" style="text-align:center;color:var(--gray);padding:30px">No businesses yet.</div>' : businesses.map(b =>`
...
showModal(` <div class="modal-head"> <div class="modal-title">Create Business</div> <button class="modal-close" onclick="closeModal()">×</button> </div> <div style="display:flex;flex-direction:column;gap:12px"> <div><div class="field-lbl">Owner Email</div> <input class="inp" id="sa-cb-email" type="email" placeholder="owner@business.com"/></div> <div><div class="field-lbl">Owner Password</div> <input class="inp" id="sa-cb-pass" type="password" placeholder="Min 6 characters"/></div> <div><div class="field-lbl">Business Name</div> <input class="inp" id="sa-cb-name" placeholder="The James Room"/></div> <div><div class="field-lbl">Admin PIN (4-6 digits)</div> <input class="inp" id="sa-cb-admin" type="number" inputmode="numeric" placeholder="e.g. 1234"/></div> <div><div class="field-lbl">Manager PIN (4-6 digits)</div> <input class="inp" id="sa-cb-mgr" type="number" inputmode="numeric" placeholder="e.g. 5678"/></div> <button class="btn btn-primary btn-full" onclick="window._saDoCreate()">Create Business</button> </div>`);
...
const tapId=sessionStorage.getItem(ck+’*id’)||’tap*’+now;
...
el.innerHTML=`<div style="text-align:center;margin-bottom:16px"><div style="font-size:18px;font-weight:800;margin-bottom:6px">${esc(b.lowRatingMsg||"We're sorry to hear that.")}</div></div> <textarea id="fb-t" style="width:100%;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.1);border-radius:14px;padding:14px;color:inherit;font-size:14px;font-family:'Nunito',sans-serif;outline:none;resize:none;min-height:100px;margin-bottom:12px" placeholder="Tell us what happened…"></textarea> <button onclick="window._fb(${r})" style="width:100%;background:${esc(b.brandColor||'#00e5a0')};color:#07080c;border:none;border-radius:14px;padding:14px;font-size:15px;font-weight:800;cursor:pointer;font-family:'Nunito',sans-serif">Submit</button>`;
...
app().innerHTML=` <style>body{background:${esc(b.bgColor||'#07080c')};color:${esc(b.textColor||'#fff')}}.star{cursor:pointer;font-size:42px;transition:transform .15s;filter:grayscale(1);opacity:.3}.star.lit{filter:none;opacity:1}.star:active{transform:scale(1.25)}</style> <div class="tap-page" style="position:relative"> ${staffBubbleHTML} <div style="margin-top:16px;margin-bottom:24px;text-align:center"> ${b.logoUrl?`<img src="${esc(b.logoUrl)}" style="height:80px;max-width:220px;object-fit:contain;border-radius:16px"/>`:`<div style="font-size:28px;font-weight:900">${esc(b.name)}</div>`} ${b.tagline?`<div style="font-size:13px;opacity:.4;margin-top:8px">${esc(b.tagline)}</div>`:''} </div> <div style="text-align:center;margin-bottom:28px;width:100%"> <div style="font-size:20px;font-weight:900;margin-bottom:20px">${esc(b.ratingQuestion||'How was your experience today?')}</div> <div style="display:flex;gap:10px;justify-content:center"> ${[1,2,3,4,5].map(i=>`<div id="cs${i}" class="star" onclick="window._cs(${i})">★</div>`).join('')} </div> </div> <div id="after" style="width:100%"></div> ${bulletinLinks.length?`<div style="width:100%;margin-top:8px"><div style="font-size:10px;font-weight:700;opacity:.3;letter-spacing:.1em;text-transform:uppercase;margin-bottom:12px;text-align:center">${esc(b.name)}</div>${bulletinLinks.map(l=>linkRow(l)).join(’’)}</div>`:''} </div> <div style="position:fixed;bottom:10px;left:0;right:0;text-align:center;font-size:9px;font-weight:700;letter-spacing:.18em;text-transform:uppercase;opacity:.08;pointer-events:none">POWERED BY TAP+</div>`;
}
}